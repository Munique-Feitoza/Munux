...
static void keyboard_handler(registers_t regs) {
    ...
    // Flag de sequência estendida (prefixo 0xE0): setas, Delete, Home/End, etc.
    static uint8_t kbd_extended = 0;

    uint8_t scancode = inb(0x60);

    // Prefixo de scancode estendido — consome e marca o próximo byte.
    if (scancode == 0xE0) {
        kbd_extended = 1;
        return;
    }
    // Byte que segue o 0xE0: tecla estendida ainda não mapeada. Ignora (make e
    // break) para não injetar um caractere espúrio no buffer de entrada.
    if (kbd_extended) {
        kbd_extended = 0;
        return;
    }
