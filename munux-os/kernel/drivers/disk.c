...
// Timeout de polling (iterações). Evita hang infinito quando não há disco.
#define ATA_TIMEOUT 1000000

// Aguarda BSY baixar. Retorna 0 se ok, -1 no timeout.
...
// Aguarda DRQ (dados prontos). Retorna 0 se ok, -1 em erro/timeout.
...
// Detecta presença do Primary Master. Sem disco, a porta de status "flutua"
// em 0xFF; com disco, reporta um status válido (ex.: DRDY). Retorna 1/0.
int disk_present(void) {
    outb(ATA_PRIMARY_DRIVE, 0xA0); // seleciona master
    io_wait();
    uint8_t status = inb(ATA_PRIMARY_STATUS);
    return (status != 0xFF && status != 0x00) ? 1 : 0;
}
